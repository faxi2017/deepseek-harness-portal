...
| `GET /api/admin/gateway/analytics` | 同一快照返回筛选后的总览、前期对比、趋势、用户/模型排行和时段热力数据 |
| `GET /api/gateway/analytics` | 当前登录用户自己的总览、趋势、模型分布、时段热力和明细；拒绝 `userId` 筛选 |
...
管理员后台“模型网关”默认打开“用量分析”，设计参考 CC Switch 的筛选驱动统计方式：日期、用户、模型和粒度共同驱动指标卡、全部图表、明细及 CSV 导出，保证同一屏口径一致。

- 快捷范围：今日、昨日、本周、本月、近 7/30/90 天、今年及自定义日期。今日自动按小时，2–90 天按日，较长范围按周；也可手动选择日/周/月，小时最多 31 天。
- 指标卡：实际 Token、平台请求、调用用户、日均实际 Token，并显示上个等长日期区间的变化。日均包含范围内的无调用日期。
- 消耗趋势：可在折线图和堆叠柱状图之间切换，选择实际 Token、额度扣减或请求次数，并按输入/输出、用户或模型对比。用户/模型对比展示前 5 位，其余合并。
- 多维图表：用户前 10 排行横向柱状图、模型占比环图、北京时间星期/小时热力图、请求结算状态堆叠柱图。点击用户柱条或模型扇区会联动筛选。
- 明细可按用户、模型、时间及“时间 × 用户 × 模型”切换，每页 20 行；CSV 导出包含当前筛选和汇总方式的全部行，并处理电子表格公式注入前缀。
- 图表库使用项目本地依赖的 Apache ECharts，不从浏览器访问公共 CDN。小屏将指标和图表改为单列，筛选控件分行。

子用户 Portal 的“模型网关”使用同一日期、模型、粒度和账务口径，但不提供用户排行或用户筛选。页面有实际 Token、请求、使用模型、日均用量指标，折线/柱状趋势、模型环图、活跃时段热力图、请求状态图、按模型/时间/明细的表格及 CSV 导出。服务端先以会话用户 ID 固定查询，再删除用户 ID、用户名和用户列表；即使手工附带别的 `userId` 参数也会直接返回 400。

统计只覆盖 Portal 平台网关请求。实际 Token 仍是供应商最终回执的输入加输出；待核实扣减和进行中预留单独展示。当前没有持久化缓存读写细分、单价、耗时和首 Token 延迟，因此不仿造 CC Switch 的缓存命中率、费用或延迟指标。若以后新增这些字段，需要从请求产生时开始记录，历史流水不会倒推出可靠值。
...
- 全部 49 项自动化测试通过，包含并发预占、权限隔离、跨日结算、重启恢复、异常输入、流中断保留预占、Bifrost 空回复项用量回执、管理员与子用户的多维统计隔离和原有 Portal 回归。
