...
更新日期：2026-09-15。此文件是本仓库唯一的项目级持续记忆，记录当前实现、用户已确认的方向和后续工作。功能改动完成时，必须在同一变更中同步更新本文件；部署步骤见 `DEPLOYMENT.md`，不要按旧版上游 README 或历史审计中的部署方式操作。
...
| DSH 批量升级/灰度/回滚 | 已支持版本记录、逐实例灰度、数据卷备份、健康检查自动回退与手动回退；暂不自动批量推送，避免未验证版本同时影响所有用户 |
| 防止绕过平台额度 | 平台凭证、模型权限和用量身份由 Portal 校验；保留用户自己的公网 Key，不将个人调用纳入平台额度 |
| 生产发布 | 尚未部署为服务器服务、未推送 Git；按 DEPLOYMENT.md 完成 |

网关与 DSH 镜像、用户 home/workspace 持久卷独立；下发只修改专用平台 provider，不覆盖个人模型或用户默认选择（新用户初始化或显式要求设默认时除外）。管理员保存默认模型和额度后可一键将该默认权限覆盖到全部普通用户，系统逐个启动停止的实例并下发，单个失败不阻断后续用户；用户列表仍可配置个别例外。未来 DSH 版本接口仍需灰度验证。只读根文件系统与用户任意安装系统软件有明确边界。
...
- 80 项自动化测试：注册登录/并发/容量失败/验证码接口移除、邀请码/关闭注册/限流、管理员重置密码与会话撤销、Docker 错误传播/生命周期、端口及 HTTP/WS 代理、模型网关权限/结算、默认配置批量覆盖与单用户失败隔离、批量下发确认后的按钮引用、管理员与子用户多维统计隔离，以及默认插件校验、安装队列、失败恢复和生命周期冲突；其中 3 项为旧邮箱事务兼容测试。
...
2026-09-07：个人模型用量同步兼容新版 DSH 已移除旧 HTTP 会话接口的情况：旧接口不可用时，从运行中容器的只含聚合数据的 `session_projcache` 快照读取累计 Token，以检查点差量幂等入库，不读取提示词或回复；管理员打开用量分析时也会同步各子用户。公共模型手工同步在服务端未启用网关时返回明确配置原因。Bifrost 启动脚本按镜像的 `1000:0` 运行身份设置数据目录为 `0770`、敏感配置为 `0660`，仅允许 root 用户/组与容器进程读写。

同日进一步修复新增平台模型和用户下发：Bifrost 的上游 Key 名称全局唯一，新模型改用包含模型 ID 的托管名称，同时兼容已存在的旧名称；DSH RPC 适配当前斜杠端点、`args` 参数信封和 Web Token 会话认证，401 时自动刷新一次内部 Cookie，Cookie 不返回浏览器或写入数据库。

平台模型下发到 DSH 时，目录中的名称使用管理员填写的上游模型 ID，内部授权 ID 仍使用不可伪造的 Portal 模型 ID。Linux 下显示的 `172.25.0.1:7999/v1` 是租户专用 bridge 的宿主机网关，只在服务器内部使用。请求预留按 JSON UTF-8 字节数的一半加协议余量和输出上限估算，避免 DSH 的系统提示/工具定义被旧算法放大为两倍字节数而在零用量时误报 429；最终仍以供应商回执的实际 Token 结算。

未验证：货币计费、所有插件的安装更新、未来 DSH 版本兼容性、生产负载、批量升级和数据版本回滚。2026-09-07 已在当前 Ubuntu 服务器启动并验证 Portal/Bifrost 基础服务、公共模型同步和现有个人模型用量补采；尚未执行新的真实模型对话端到端调用。
