...
更新日期：2026-09-03。此文件记录本仓库当前实现、用户已确认的方向和后续工作。部署步骤见 `DEPLOYMENT.md`；不要按旧版上游 README 或历史审计中的部署方式操作。
...
没有 Docker Compose、模型网关、独立前端构建、CI 发布流水线或批量升级队列。Portal 使用 Docker CLI 控制同一台主机的实例，当前应只运行一个 Portal 进程。
...
专属网络禁用 ICC、IPv6；防火墙助手短暂使用 host 网络和 NET_ADMIN，把规则限定到本项目子网。租户可访问公网，不能访问宿主机/私网（包括内网模型服务）。启动先验证 rootful Linux Docker、镜像、网络和 `DOCKER-USER` 链；失败则拒绝启动。重启 Docker 后应重启 Portal 重新应用规则。目前不支持 rootless Docker 或原生 nftables 后端。
...
| 用户自行管理插件 | DSH 自身功能 + 持久化目录；具体插件需验证权限和兼容性 |
| 用户一键重启 DSH | 当前有本人 start/stop API，用户页仅 Launch；需补原子 restart 操作及按钮 |
| 管理员重置密码 | 已有 API，本次补齐 Users 页面按钮 |
| 统一模型 API Key | 未实现；需要网关及服务端配置，租户只获得网关令牌 |
| 每人每天限额 | 未实现；需确认按 token、费用或次数计量、重置时区、流式/并发结算、超限行为 |
| 精确用量统计 | 现有 request_count 只是代理请求数，不是模型调用数或 token 消耗 |
| DSH 批量升级/灰度/回滚 | 只有逐实例 Reprovision；需镜像版本记录、任务队列、进度、备份及回滚策略 |
| 防止绕过统一额度 | 当前租户有公网访问与自带 Key；若要求强制管控，需要网关与出站策略共同限制 |
| 生产发布 | 尚未部署为服务器服务、未推送 Git；按 DEPLOYMENT.md 完成 |

后续先确定模型网关选型和额度规则；不应把旧对话里“很容易实现”的表述当作已完成能力。只读根文件系统与用户任意安装系统软件有明确边界。
...
- 18 项自动化测试：注册登录/并发/容量失败/验证码接口移除、邀请码/关闭注册/限流、管理员重置密码与会话撤销、Docker 错误传播/生命周期、端口及 HTTP/WS 代理；其中 3 项为旧邮箱事务兼容测试。
...
未验证：真实模型 Key 调用和计费、所有插件的安装更新、生产负载、Ubuntu 上 Portal 服务完整部署、批量升级和数据版本回滚。服务器仅检查环境并缓存了基础镜像，没有替换现有业务或重启 Docker。
